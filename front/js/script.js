...
function startGame(operation) {
  ...
  switch (operation) {
    case 'addition':
      ...
      definition =
        'A adição é a operação matemática que representa a soma de dois números.';
      break;
    case 'subtraction':
      operationName = 'Definição de Subtração';
      definition =
        'A subtração é a operação matemática que representa a diferença entre dois números.';
      break;
    case 'multiplication':
      operationName = 'Definição de Multiplicação';
      definition =
        'A multiplicação é a operação matemática que representa a soma repetida de um número.';
      break;
    case 'division':
      operationName = 'Definição de Divisão';
      definition =
        'A divisão é a operação matemática que representa a partição de um número em partes iguais.';
      break;
  }

  document.getElementById(
    'operation-definition'
  ).textContent = `${operationName}: ${definition}`;
...
function checkAnswer(answer, correctAnswerIndex) {
  if (
    answer ===
    questions[currentCategory][currentQuestionIndex].answers[correctAnswerIndex]
  ) {
    score++;
...
function endGame() {
  fetch('http://localhost:3000/pontuacoes', {
    ...
    .catch((error) => {
      ...
      // Mostrar resultado mesmo se houver erro na API
      showResult({ jogador: playerName, pontos: score });
    });
}
...
function showResult(data) {
  ...
  document.getElementById(
    'final-message'
  ).textContent = `Você acertou ${data.pontos} de ${questions[currentCategory].length} perguntas!`;
